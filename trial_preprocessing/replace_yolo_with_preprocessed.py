"""
Replace existing YOLO data with slices from preprocessed nnU-Net data
Reads .b2nd files and extracts slices with proper normalization
"""
...
def load_nnunet_data(case_file):
    """Load data from nnU-Net .b2nd format"""
    import blosc2

    # Load image data
    data_file = case_file.parent / f"{case_file.stem}.b2nd"
    seg_file = case_file.parent / f"{case_file.stem}_seg.b2nd"

    # Load using blosc2
    data_array = blosc2.open(str(data_file))
    seg_array = blosc2.open(str(seg_file))

    # Convert to numpy
    data = np.array(data_array)
    seg = np.array(seg_array)
...
# REPLACE existing YOLO data
TRAIN_IMAGES_OUT = Path(r"C:\Users\anoma\Downloads\yolo-4-scSE\images\train")
TRAIN_LABELS_OUT = Path(r"C:\Users\anoma\Downloads\yolo-4-scSE\labels\train")
VAL_IMAGES_OUT = Path(r"C:\Users\anoma\Downloads\yolo-4-scSE\images\val")
VAL_LABELS_OUT = Path(r"C:\Users\anoma\Downloads\yolo-4-scSE\labels\val")
...
def process_case(case_id, images_out_dir, labels_out_dir):
    ...
    data_file = PREPROCESSED_DIR / f"{case_id}.b2nd"
    ...
    try:
        ...
        data, seg = load_nnunet_data(data_file)
        ...
        volume_normalized = normalize_for_yolo(data)
...
print("\n" + "="*70)
print("PROCESSING TRAINING CASES (PREPROCESSED DATA)")
print("="*70)
train_slices = 0
...
print("\n" + "="*70)
print("PROCESSING VALIDATION CASES (PREPROCESSED DATA)")
print("="*70)
val_slices = 0
...
print("\nYOLO data has been REPLACED with preprocessed slices!")
print("="*70)
