...
def create_new_affine(original_affine, original_spacing, target_spacing):
    ...
    Args:
        ...
        original_spacing: Original (x, y, z) spacing
        target_spacing: Target (x, y, z) spacing

    Returns:
        new_affine: Updated 4x4 affine matrix
    """
...
def create_sanctuary_for_dataset(dataset_config, dry_run=True):
    ...
    Returns:
        all_stats: List of processing statistics (None for dry_run)
    """
    ...
    if not volume_files:
        print(f"WARNING: No volume files found in {volumes_dir}")
        return None
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python sancturary_spacings.py --dry-run    # Preview what will be created
  python sancturary_spacings.py              # Create sanctuary folders
  ...
  Original high-quality detail is preserved where it exists
    ...
    for dataset_config in DATASETS:
        if not dataset_config['base_path'].exists():
            print(f"\nWARNING: Dataset not found: {dataset_config['base_path']}")
