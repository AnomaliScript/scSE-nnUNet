...
class SurgicalCorridorOptimizedOctree:
    ...
    def _has_critical_neighbors(self, seg_region: np.ndarray,
        ...
        NOTE: Currently disabled because segmentation only has VERTEBRAL_BODY
        This would require pedicle/canal/artery/nerve segmentation labels
        """
        # COMMENTED OUT: These anatomical regions don't exist in current segmentation model
        # critical_structures = {
        ...
        # SIMPLIFIED: Just check if there are multiple different labels (any boundary)
        unique_labels = set(AnatomicalRegion(int(l)) for l in np.unique(seg_region))

        # If more than one label type, it's a boundary (e.g., bone vs background)
        return len(unique_labels) > 1

        # Original code (requires multi-label segmentation):
        # unique_labels = set(AnatomicalRegion(int(l)) for l in np.unique(seg_region))
...
class SurgicalCorridorAnalyzer:
    ...
    def _analyze_critical_proximity(self) -> Dict:
        ...
        NOTE: Simplified because segmentation only has VERTEBRAL_BODY
        """
        # COMMENTED OUT: These labels don't exist in current segmentation model
        # critical_labels = [
        #     AnatomicalRegion.SPINAL_CANAL,
        #     AnatomicalRegion.VERTEBRAL_ARTERY,
        #     AnatomicalRegion.NERVE_ROOT
        ...
        # SIMPLIFIED: Just analyze vertebral body and background
        available_labels = [
            AnatomicalRegion.BACKGROUND,
            AnatomicalRegion.VERTEBRAL_BODY
        ]
...
def create_synthetic_cervical_spine(shape=(128, 128, 64)):
    ...
    NOTE: Simplified to only create VERTEBRAL_BODY segmentation
    (matches what the actual segmentation model produces)
    """
    ...
    # COMMENTED OUT: Pedicles (don't exist in actual segmentation model)
    ...
    # COMMENTED OUT: Spinal canal (doesn't exist in actual segmentation)
    ...
    # Create importance map - SIMPLIFIED (only vertebral body matters)
    importance_map = np.zeros(shape, dtype=np.float32)
    # importance_map[segmentation == AnatomicalRegion.PEDICLE.value] = 0.95  # COMMENTED OUT
    # importance_map[segmentation == AnatomicalRegion.SPINAL_CANAL.value] = 0.9  # COMMENTED OUT
