...
class AnatomicalRegion(Enum):
    """
    Cervical vertebrae labels (C1-C7)
    NOVEL: Per-vertebra adaptive resolution for surgical planning

    This version enables vertebra-specific mesh detail allocation:
    - High detail for target vertebrae (C4-C6: common screw placement sites)
    - Medium detail for adjacent vertebrae (C1-C3, C7)
    - Background at minimal detail

    Compatible with segmentation models that output individual vertebra labels.
    """
    BACKGROUND = 0
    C1_VERTEBRA = 1  # Atlas - unique anatomy
    C2_VERTEBRA = 2  # Axis - odontoid process
    C3_VERTEBRA = 3  # Standard cervical vertebra
    C4_VERTEBRA = 4  # Common screw target
    C5_VERTEBRA = 5  # MOST COMMON screw target - highest detail
...
# NOVEL: Per-vertebra importance weighting for adaptive mesh resolution
# Target vertebrae (C4-C6) receive highest detail for screw placement planning
ANATOMICAL_CRITICALITY_PROFILES: Dict[AnatomicalRegion, AnatomicalCriticalityScore] = {
    ...
    # Adjacent vertebrae - medium importance (less common surgical targets)
    AnatomicalRegion.C1_VERTEBRA: AnatomicalCriticalityScore(
        0.5, 0.2, 0.2,
        {SurgicalPhase.PLANNING: 0.8}
    ),
    AnatomicalRegion.C2_VERTEBRA: AnatomicalCriticalityScore(
        0.5, 0.2, 0.2,
        {SurgicalPhase.PLANNING: 0.8}
    ),
    AnatomicalRegion.C3_VERTEBRA: AnatomicalCriticalityScore(
        0.5, 0.2, 0.2,
        {SurgicalPhase.PLANNING: 0.8}
    ),

    # Target vertebrae - HIGH importance (C4-C6: common screw placement sites)
    AnatomicalRegion.C4_VERTEBRA: AnatomicalCriticalityScore(
        0.9, 0.4, 0.4,
        {
    ...
    AnatomicalRegion.C5_VERTEBRA: AnatomicalCriticalityScore(
        1.0, 0.5, 0.5,  # HIGHEST priority - most common screw target
        {
            SurgicalPhase.PLANNING: 1.5,
            SurgicalPhase.PEDICLE_IDENTIFICATION: 1.8,
            SurgicalPhase.SCREW_TRAJECTORY: 2.0,
            SurgicalPhase.SCREW_PLACEMENT: 2.5
        }
    ),
    AnatomicalRegion.C6_VERTEBRA: AnatomicalCriticalityScore(
        0.9, 0.4, 0.4,
        {
            SurgicalPhase.PLANNING: 1.2,
            SurgicalPhase.PEDICLE_IDENTIFICATION: 1.5,
    ...
    # Lower cervical - moderate importance
    AnatomicalRegion.C7_VERTEBRA: AnatomicalCriticalityScore(
        0.5, 0.2, 0.2,
        {SurgicalPhase.PLANNING: 0.8}
    ),
}
...
def test_anatomical_criticality():
    ...
    # Test C5 vertebra (highest priority target for screw placement)
    c5_score = ANATOMICAL_CRITICALITY_PROFILES[AnatomicalRegion.C5_VERTEBRA]

    print("\nC5 Vertebra (Primary Screw Target) - Phase-Dependent Importance:")
    for phase in SurgicalPhase:
        importance = c5_score.compute_total_importance(phase)
        print(f"  {phase.value:20s}: importance = {importance:.3f}")

    # Test C1 vertebra (lower priority)
    c1_score = ANATOMICAL_CRITICALITY_PROFILES[AnatomicalRegion.C1_VERTEBRA]
    print("\nC1 Vertebra (Adjacent Vertebra) - Phase-Dependent Importance:")
    for phase in SurgicalPhase:
        importance = c1_score.compute_total_importance(phase)
        print(f"  {phase.value:20s}: importance = {importance:.3f}")
...
def extract_surface_mesh(volume, threshold=0.5, spacing=(1.0, 1.0, 1.0),
                        phase=None, segmentation=None, importance_map=None, uncertainty_map=None):
    """
    ...
    Args:
        ...
        segmentation: Optional segmentation labels (C1-C7), if not provided creates default
    ...
    # Create default segmentation if not provided (all C5 for maximum detail)
    if segmentation is None:
        segmentation = np.ones_like(volume, dtype=np.int32) * AnatomicalRegion.C5_VERTEBRA.value
...
def create_pyvista_mesh(vertices, faces):
    ...
    if len(vertices) == 0 or len(faces) == 0:
        print("Warning: Empty mesh (0 vertices or faces)")
        return pv.PolyData()
