...
import time
from .marching_cubes_core_data import MarchingCubesLookupTables, UncertaintyAwareInterpolation, AnatomicalRegion
...
class MeshQualityValidator:
    ...
    def validate(self) -> MeshQualityMetrics:
        ...
        # Anatomical coverage - count vertices per vertebra

        # Count target vertebrae vertices (C4-C6: primary screw placement sites)
        # Using 'pedicle_vertex_count' name for backward compatibility but it now
        # represents high-priority target vertebrae
        metrics.pedicle_vertex_count = sum(
            1 for v in self.vertices
            if v.anatomical_label in [
                AnatomicalRegion.C4_VERTEBRA.value,
                AnatomicalRegion.C5_VERTEBRA.value,
                AnatomicalRegion.C6_VERTEBRA.value
            ]
        )

        # Count adjacent vertebrae (C1-C3, C7: lower surgical priority)
        # Using 'critical_structure_vertex_count' name for backward compatibility but it now
        # represents adjacent/lower-priority vertebrae
        metrics.critical_structure_vertex_count = sum(
            1 for v in self.vertices
            if v.anatomical_label in [
                AnatomicalRegion.C1_VERTEBRA.value,
                AnatomicalRegion.C2_VERTEBRA.value,
                AnatomicalRegion.C3_VERTEBRA.value,
                AnatomicalRegion.C7_VERTEBRA.value
            ]
        )
